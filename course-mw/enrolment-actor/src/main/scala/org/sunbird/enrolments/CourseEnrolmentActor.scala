...
import java.util
import java.util.{Comparator, Date, Optional}
                ...
                for (courseId <- courseIds) {
                    ...
                    val participants: java.util.List[java.util.Map[String, AnyRef]] = {
                        if (CollectionUtils.isNotEmpty(participantListByCourseId)) {
                            ...
                            // populate user data
                            for (userId <- userIds) {
                                val activeUserDetails: util.List[java.util.Map[String, AnyRef]]
                                = userCoursesService.getParticipantsDetails(userId, true, request.getRequestContext)
                                if (isNotNull(activeUserDetails)) {
                                    participantListByCourseId.map(x => {
                                        if (x.get(JsonKey.USER_ID) == activeUserDetails.get(0).get(JsonKey.USER_ID)) {
                                            x.put(JsonKey.FIRST_NAME, activeUserDetails.get(0).get(JsonKey.FIRST_NAME))
                                            x.put(JsonKey.LAST_NAME, activeUserDetails.get(0).get(JsonKey.LAST_NAME))
